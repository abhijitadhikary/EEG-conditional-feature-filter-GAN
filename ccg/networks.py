...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if which_model_netG == 'resnet_9blocks':
        netG = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9, activation=activation)
    ...
    print(netG)
    # summary (netG, np.shape(input_nc))
    # assert (0)
...
def define_D(input_nc, ndf, which_model_netD, n_layers_D=3, norm='batch', activation='none', init_type='normal', gpu_ids=[], internal_cond_D=False, num_classes=None):
    ...
    if not internal_cond_D:
        if which_model_netD == 'basic':
            netD = NLayerDiscriminator(input_nc, ndf, n_layers=3, norm_layer=norm_layer, activation=activation)
        elif which_model_netD == 'n_layers':
            netD = NLayerDiscriminator(input_nc, ndf, n_layers_D, norm_layer=norm_layer, activation=activation)
        elif which_model_netD == 'pixel':
            netD = PixelDiscriminator(input_nc, ndf, norm_layer=norm_layer, activation=activation)
        else:
            raise NotImplementedError('Discriminator model name [%s] is not recognized' %
                                      which_model_netD)
    else:
        netD = ConditionalDiscriminator(input_nc, ndf, n_layers=3, norm_layer=norm_layer, activation=activation, num_classes=num_classes)
    # print (netD)
    # assert (0)
...
def create_model_classifier(classifier_name, num_classes, activation='none'):
    ...
    if activation=='none':
        # TODO remove sigmoid from last layer
...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
                 padding_type='reflect', activation=True):
        assert (n_blocks >= 0)
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, activation='none'):
...
class PixelDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d, activation=False):
...
class Classifier(nn.Module):
    def __init__(self, input_nc, num_classes, ndf, norm_layer=nn.BatchNorm2d):
...
class ConditionalDiscriminator(nn.Module):
    ...
    '''
    def __init__(self, input_nc, ndf, n_layers=3, norm_layer=nn.BatchNorm2d, activation='none', num_classes=None):
        ...
        self.block_4 = nn.Sequential(
            ...
            nn.Conv2d(in_channels=ndf * 4, out_channels=1, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, input, label):
        ...
        if label is not None:
            ...
            out = self.block_4(out).reshape(batch_size, -1)
            one_hot_labels = torch.zeros((batch_size, self.num_classes), dtype=out.dtype).to(out.device)
            for index_batch in range(batch_size):
                current_label = label[index_batch]
                one_hot_labels[index_batch, current_label] = 1
