...
class CycleGANSemanticModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf,
                                            ...
                                            opt.n_layers_D, opt.norm, use_sigmoid, 
                                            opt.init_type, self.gpu_ids)
            self.netCLS = networks.define_C(opt.output_nc, opt.num_classes, opt.ndf, 
    ...
    def set_input(self, input, opt):
        ...
        if 'A_label_alcoholism' in input and 'B_label_alcoholism' in input:
            self.input_A_label_alcoholism = input['A_label_alcoholism' if AtoB else 'B_label_alcoholism'].to(self.device)
            self.input_B_label_alcoholism = input['B_label_alcoholism' if AtoB else 'A_label_alcoholism'].to(self.device)
            #self.image_paths = input['B_paths'] # Hack!! forcing the labels to corresopnd to B domain
            
            
        if 'A_label_id' in input and 'B_label_id' in input:
            self.input_A_label_id = input['A_label_id' if AtoB else 'B_label_id'].to(self.device)
            self.input_B_label_id = input['B_label_id' if AtoB else 'A_label_id'].to(self.device)        

            
        if 'A_label_stimulus' in input and 'B_label_stimulus' in input:
            self.input_A_label_stimulus = input['A_label_stimulus' if AtoB else 'B_label_stimulus'].to(self.device)
            self.input_B_label_stimulus = input['B_label_stimulus' if AtoB else 'A_label_stimulus'].to(self.device) 
            
        if 'A_label_combined' in input and 'B_label_combined' in input:
    ...
    def backward_CLS(self):
        #label_A = self.input_A_label_stimulus
        label_A = self.input_A_label
        # forward only real source image through semantic classifier
        pred_A = self.netCLS(self.real_A) 

        #label_B = self.input_B_label_stimulus
        label_B = self.input_B_label

        pred_B = self.netCLS(self.real_B)

        self.loss_CLS = self.criterionCLS(pred_A, label_A.long()) #+ self.criterionCLS(pred_B, label_B.long()) 
    ...
    def backward_G(self):
        ...
        # semantic loss AB
        #print ( (self.pred_fake_B[0]))
        #print ( (self.input_A_label[0]))
        #print (self.pred_fake_B)
        
        #self.loss_sem_AB = self.criterionCLS(self.pred_fake_B, self.input_A_label_stimulus.long())
        ...
        # semantic loss BA
        self.loss_sem_BA = self.criterionCLS(self.pred_fake_A, self.gt_pred_B.long())
