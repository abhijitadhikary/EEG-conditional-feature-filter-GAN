...
class ClassificationNet(nn.Module):
    def __init__(self):
        ...
        #self.fc4 = torch.nn.Linear(in_features = 64,  out_features = 2)
...
if __name__ == '__main__':
    ...
    #mat = scipy.io.loadmat('uci_eeg_images_class.mat')
    ...
    y_train = data_train['label_alcoholism'].astype('int') 
    y_train = y_train.reshape(np.shape(y_train)[0])
    y_test = data_test['label_alcoholism'].astype('int')
    ...
    for i in range(EPOCH):
        ...
        if test_correct/len(test_dataset) > max_acc:
            ...
            torch.save(net.state_dict(), 'autoencoder_checkpoints/final_classification_model_disease_within_augmented.pkl')
            torch.save(Image_AC.state_dict(), 'autoencoder_checkpoints/Image-wise_autoencoders_disease_within_augmented.pkl')
