...
class ClassificationNet(nn.Module):
    def __init__(self):
        ...
        self.fc2 = torch.nn.Linear(in_features = 512,  out_features = 122)
        #self.fc3 = torch.nn.Linear(in_features = 128,  out_features = 120)
...
if __name__ == '__main__':
    ...
    data_train = sio.loadmat(data_root+'uci_eeg_images_train_within.mat')
    ...
    y_train = data_train['label_id'].astype('int') 
    y_train = y_train.reshape(np.shape(y_train)[0])
    y_test = data_test['label_id'].astype('int')
    ...
    for i in range(EPOCH):
        ...
        if test_correct/len(test_dataset) > max_acc:
            ...
            torch.save(net.state_dict(), 'autoencoder_checkpoints/final_classification_model_id.pkl')
            torch.save(Image_AC.state_dict(), 'autoencoder_checkpoints/Image-wise_autoencoders_within_id.pkl')
